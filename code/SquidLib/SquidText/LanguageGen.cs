...
namespace SquidLib.SquidText {
    public class LanguageGen {
        public LanguageGen(string[] openingVowels, string[] midVowels, string[] openingConsonants,
            ...
            openingVowels = openingVowels ?? Array.Empty<string>();
            midVowels = midVowels ?? Array.Empty<string>();
            openingConsonants = openingConsonants ?? Array.Empty<string>();
            midConsonants = midConsonants ?? Array.Empty<string>();
            closingConsonants = closingConsonants ?? Array.Empty<string>();
            closingSyllables = closingSyllables ?? Array.Empty<string>();
            vowelSplitters = vowelSplitters ?? Array.Empty<string>();
            syllableLengths = syllableLengths ?? Array.Empty<int>();
            syllableFrequencies = syllableFrequencies ?? Array.Empty<double>();

            this.openingVowels = openingVowels;
            this.midVowels = (new string[openingVowels.Length + midVowels.Length]);
            Array.Copy(midVowels, 0, this.GetMidVowels(), 0, midVowels.Length);
            Array.Copy(openingVowels, 0, this.GetMidVowels(), midVowels.Length, openingVowels.Length);
            this.openingConsonants = openingConsonants;
            this.midConsonants = (new string[midConsonants.Length + closingConsonants.Length]);
            Array.Copy(midConsonants, 0, this.GetMidConsonants(), 0, midConsonants.Length);
            Array.Copy(closingConsonants, 0, this.GetMidConsonants(), midConsonants.Length, closingConsonants.Length);
            ...
            for (int i = 0; i < syllableLengths.Length; i++) {
                TotalSyllableFrequency += (this.GetSyllableFrequencies()[syllableLengths[i] - 1] = syllableFrequencies[i]);
            }
        ...
        internal LanguageGen(LanguageGen other) {
            openingVowels = ArrayTools.Copy(other.GetOpeningVowels());
            midVowels = ArrayTools.Copy(other.GetMidVowels());
            openingConsonants = ArrayTools.Copy(other.GetOpeningConsonants());
            midConsonants = ArrayTools.Copy(other.GetMidConsonants());
            closingConsonants = ArrayTools.Copy(other.GetClosingConsonants());
            vowelSplitters = ArrayTools.Copy(other.GetVowelSplitters());
            closingSyllables = ArrayTools.Copy(other.GetClosingSyllables());
            syllableFrequencies = ArrayTools.Copy(other.GetSyllableFrequencies());
            ...
            Clean = other.Clean;
            sanityChecks = other.GetSanityChecks();
            Modifiers = new List<Modifier>(other.Modifiers);
        }

        private readonly string[] openingVowels;
        public string[] GetOpeningVowels() => openingVowels;

        private readonly string[] midVowels;
        public string[] GetMidVowels() => midVowels;

        private readonly string[] openingConsonants;
        public string[] GetOpeningConsonants() => openingConsonants;

        private readonly string[] midConsonants;
        public string[] GetMidConsonants() => midConsonants;

        private readonly string[] closingConsonants;
        public string[] GetClosingConsonants() => closingConsonants;

        private readonly string[] vowelSplitters;
        public string[] GetVowelSplitters() => vowelSplitters;

        private readonly string[] closingSyllables;
        public string[] GetClosingSyllables() => closingSyllables;

        private readonly double[] syllableFrequencies;
        public double[] GetSyllableFrequencies() => syllableFrequencies;
        ...
        private readonly Regex[] sanityChecks;
        public Regex[] GetSanityChecks() => sanityChecks;
        ...
        private static readonly Replacer[] accentFinders = new Replacer[] {
        ...
        public static Replacer[] GetAccentFinders() => accentFinders;
        ...
        accentedConsonants = new char[][] {
            ...
            new char[]{'j', 'ĵ', 'ȷ',},
            new char[]{'k', 'ķ',},
            new char[]{'l', 'ĺ', 'ļ', 'ľ', 'ŀ', 'ł',},
            new char[]{'m',},
            new char[]{'n', 'ñ', 'ń', 'ņ', 'ň', 'ŋ',},
            new char[]{'p',},
            new char[]{'q',},
            new char[]{'r', 'ŕ', 'ŗ', 'ř',},
            new char[]{'s', 'ś', 'ŝ', 'ş', 'š', 'ș',},
            new char[]{'t', 'ţ', 'ť', 'ț',},
            new char[]{'v',},
            new char[]{'w', 'ŵ', 'ẁ', 'ẃ', 'ẅ',},
            new char[]{'x',},
...
*/
        ...
        public static string RemoveAccents(string str) {
            ...
            for (int i = 0; i < GetAccentFinders().Length; i++) {
                alteredString = GetAccentFinders()[i].Replace(alteredString);
            }
        ...
        public LanguageGen RemoveAccents() {
            ...
            string[] ov = ArrayTools.Copy(GetOpeningVowels()),
                    mv = ArrayTools.Copy(GetMidVowels()),
                    oc = ArrayTools.Copy(GetOpeningConsonants()),
                    mc = ArrayTools.Copy(GetMidConsonants()),
                    cc = ArrayTools.Copy(GetClosingConsonants()),
                    cs = ArrayTools.Copy(GetClosingSyllables());
            for (int i = 0; i < ov.Length; i++) {
                ov[i] = RemoveAccents(GetOpeningVowels()[i]);
            }
            for (int i = 0; i < mv.Length; i++) {
                mv[i] = RemoveAccents(GetMidVowels()[i]);
            }
            for (int i = 0; i < oc.Length; i++) {
                oc[i] = RemoveAccents(GetOpeningConsonants()[i]);
            }
            for (int i = 0; i < mc.Length; i++) {
                mc[i] = RemoveAccents(GetMidConsonants()[i]);
            }
            for (int i = 0; i < cc.Length; i++) {
                cc[i] = RemoveAccents(GetClosingConsonants()[i]);
            }
            for (int i = 0; i < cs.Length; i++) {
                cs[i] = RemoveAccents(GetClosingSyllables()[i]);
            }
            int[] syllableCounts = new int[GetSyllableFrequencies().Length];
            double[] syllableFrequencies = ArrayTools.Copy(GetSyllableFrequencies());
            for (int i = 0; i < syllableCounts.Length; i++) {
                syllableCounts[i] = i + 1;
            }

            LanguageGen lang = new LanguageGen(ov, mv, oc, mc, cc, cs, GetVowelSplitters(), syllableCounts, syllableFrequencies,
                    VowelStartFrequency,
                    VowelEndFrequency,
                    VowelSplitFrequency,
                    SyllableEndFrequency, GetSanityChecks(), Clean);
        ...
        public LanguageGen MixPair(LanguageGen other, double otherInfluence) {
            ...
            string[] ov = Merge1000(rng, GetOpeningVowels(), other.GetOpeningVowels(), otherInfluence),
                    mv = Merge1000(rng, GetMidVowels(), other.GetMidVowels(), otherInfluence),
                    oc = Merge1000(rng, GetOpeningConsonants(), other.GetOpeningConsonants(), otherInfluence *
                            Math.Max(0.0, Math.Min(1.0, 1.0 - other.VowelStartFrequency + VowelStartFrequency))),
                    mc = Merge1000(rng, GetMidConsonants(), other.GetMidConsonants(), otherInfluence),
                    cc = Merge1000(rng, GetClosingConsonants(), other.GetClosingConsonants(), otherInfluence *
                            Math.Max(0.0, Math.Min(1.0, 1.0 - other.VowelEndFrequency + VowelEndFrequency))),
                    cs = Merge1000(rng, GetClosingSyllables(), other.GetClosingSyllables(), otherInfluence *
                            Math.Max(0.0, Math.Min(1.0, other.SyllableEndFrequency - SyllableEndFrequency))),
                    splitters = Merge1000(rng, GetVowelSplitters(), other.GetVowelSplitters(), otherInfluence);

            double[] fr = new double[Math.Max(GetSyllableFrequencies().Length, other.GetSyllableFrequencies().Length)];
            int[] syllableCounts = new int[fr.Length];
            for (int i = 0; i < fr.Length; i++) {
                syllableCounts[i] = i + 1;
            }
            Array.Copy(GetSyllableFrequencies(), 0, fr, 0, GetSyllableFrequencies().Length);
            for (int i = 0; i < other.GetSyllableFrequencies().Length; i++) {
                fr[i] += other.GetSyllableFrequencies()[i];
            }
            ...
            LanguageGen lang = new LanguageGen(ov, mv, oc, mc, cc, cs, splitters, syllableCounts, fr,
                ...
                GetSanityChecks() ?? other.GetSanityChecks(), true);
            lang.Modifiers.AddRange(Modifiers);
        ...
        public string Word(IRNG rng, bool capitalize, Regex[] additionalChecks) {
            ...
            while (true) {
                ...
                int syllables = 1, i = 0;
                for (int s = 0; s < GetSyllableFrequencies().Length; s++) {
                    if (syllableChance < GetSyllableFrequencies()[s]) {
                        syllables = s + 1;
                        break;
                    } else {
                        syllableChance -= GetSyllableFrequencies()[s];
                    }
                }
                if (rng.NextDouble() < VowelStartFrequency) {
                    sb.Append(rng.RandomElement(GetOpeningVowels()));
                    if (syllables == 1)
                        sb.Append(rng.RandomElement(GetClosingConsonants()));
                    else
                        sb.Append(rng.RandomElement(GetMidConsonants()));
                    i++;
                } else {
                    sb.Append(rng.RandomElement(GetOpeningConsonants()));
                }
                string close = "";
                bool redouble = false;
                if (i < syllables) {
                    if (rng.NextDouble() < SyllableEndFrequency) {
                        close = rng.RandomElement(GetClosingSyllables());
                        ...
                        if (!close.Contains("@"))
                            ...
                            ender.Append(rng.RandomElement(GetMidVowels()));
                            if (rng.NextDouble() < VowelSplitFrequency) {
                                ender.Append(rng.RandomElement(GetVowelSplitters()))
                                        .Append(rng.RandomElement(GetMidVowels()));
                            }
                        }
                    } else {
                        ender.Append(rng.RandomElement(GetMidVowels()));
                        ...
                        if (rng.NextDouble() >= VowelEndFrequency) {
                            ender.Append(rng.RandomElement(GetClosingConsonants()));
                            if (rng.NextDouble() < SyllableEndFrequency) {
                                close = rng.RandomElement(GetClosingSyllables());
                                if (close.Contains("@") && (syllables & 1) == 0) {
                                    redouble = true;
                ...
                for (; i < syllables; i++) {
                    sb.Append(rng.RandomElement(GetMidVowels()));
                    if (rng.NextDouble() < VowelSplitFrequency) {
                        sb.Append(rng.RandomElement(GetVowelSplitters()))
                                .Append(rng.RandomElement(GetMidVowels()));
                    }
                    sb.Append(rng.RandomElement(GetMidConsonants()));
                }
                ...
                if (GetSanityChecks() != null && !CheckAll(str, GetSanityChecks()))
                    continue;
        ...
        public string Word(IRNG rng, bool capitalize, int lowerSyllables, int upperSyllables, Regex[] additionalChecks) {
            ...
            while (true) {
                ...
                if (rng.NextDouble() < VowelStartFrequency) {
                    sb.Append(rng.RandomElement(GetOpeningVowels()));
                    if (approxSyllables == 1 && GetClosingConsonants().Length > 0)
                        sb.Append(rng.RandomElement(GetClosingConsonants()));
                    else if (GetMidConsonants().Length > 0)
                        sb.Append(rng.RandomElement(GetMidConsonants()));
                    i++;
                } else if (GetOpeningConsonants().Length > 0) {
                    sb.Append(rng.RandomElement(GetOpeningConsonants()));
                }
                string close = "";
                bool redouble = false;
                if (i < approxSyllables) {
                    if (GetClosingSyllables().Length > 0 && rng.NextDouble() < SyllableEndFrequency) {
                        close = rng.RandomElement(GetClosingSyllables());
                        ...
                        if (!close.Contains("@"))
                            ...
                            ender.Append(rng.RandomElement(GetMidVowels()));
                            if (GetVowelSplitters().Length > 0 && rng.NextDouble() < VowelSplitFrequency) {
                                ender.Append(rng.RandomElement(GetVowelSplitters()))
                                        .Append(rng.RandomElement(GetMidVowels()));
                            }
                        }
        ...
        public LanguageGen AddAccents(double vowelInfluence, double consonantInfluence) {
            ...
            string[] ov = AccentVowels(rng, GetOpeningVowels(), vowelInfluence),
                    mv = AccentVowels(rng, GetMidVowels(), vowelInfluence),
                    oc = AccentConsonants(rng, GetOpeningConsonants(), consonantInfluence),
                    mc = AccentConsonants(rng, GetMidConsonants(), consonantInfluence),
                    cc = AccentConsonants(rng, GetClosingConsonants(), consonantInfluence),
                    cs = AccentBoth(rng, GetClosingSyllables(), vowelInfluence, consonantInfluence);
            ...
            if (rng.NextDouble() < 0.8) {
                for (int i = 0; i < sz0; i++) {
                    ...
                    for (int j = 0; j < p0s; j++) {
                        ...
                        if (uas.Equals(k, StringComparison.InvariantCulture)) // uas is never null, always length 1
                            v = parts0[Value.At, j];
                        else {
...
"public static readonly LanguageGen $2 = new LanguageGen$1.R$3;"))
        ...
        public static readonly LanguageGen Lovecraft = new LanguageGen(
                ...
                new string[] { "'", "-" },
                new int[] { 1, 2, 3, 4 }, new double[] { 5, 7, 3, 2 }, 0.4, 0.31, 0.07, 0.04, null, true)
            .Register("Lovecraft");
        ...
        public static readonly LanguageGen GreekRomanized = new LanguageGen(
                ...
                Array.Empty<string>(), new int[] { 1, 2, 3, 4 }, new double[] { 5, 7, 4, 1 }, 0.45, 0.45, 0.0, 0.2, null, true)
            .Register("Greek Romanized");

        public static readonly LanguageGen GreekAuthentic = new LanguageGen(
        ...
        public static readonly LanguageGen French = new LanguageGen(
                ...
                Array.Empty<string>(), new int[] { 1, 2, 3 }, new double[] { 15, 7, 2 }, 0.35, 1.0, 0.0, 0.4, null, true)
            .Register("French");

        public static readonly LanguageGen RussianRomanized = new LanguageGen(
                ...
                Array.Empty<string>(), new int[] { 1, 2, 3, 4, 5, 6 }, new double[] { 4, 5, 6, 5, 3, 1 }, 0.1, 0.2, 0.0, 0.12, null, true)
            .Register("Russian Romanized");

        public static readonly LanguageGen RussianAuthentic = new LanguageGen(
                ...
                Array.Empty<string>(), new int[] { 1, 2, 3, 4, 5, 6 }, new double[] { 4, 5, 6, 5, 3, 1 }, 0.1, 0.2, 0.0, 0.12, null, true)
            .Register("Russian Authentic");

        public static readonly LanguageGen JapaneseRomanized = new LanguageGen(
                ...
                Array.Empty<string>(), new int[] { 1, 2, 3, 4, 5 }, new double[] { 5, 4, 5, 4, 3 }, 0.3, 0.9, 0.0, 0.07, JapaneseSanityChecks, true)
            .Register("Japanese Romanized");
        ...
        public static readonly LanguageGen Swahili = new LanguageGen(
                ...
                new string[] { "" },
                new string[] { "-@" },
                Array.Empty<string>(), new int[] { 1, 2, 3, 4, 5, 6 }, new double[] { 3, 8, 6, 9, 2, 2 }, 0.2, 1.0, 0.0, 0.12, null, true)
            .Register("Swahili");
        ...
        public static readonly LanguageGen Somali = new LanguageGen(
                ...
                Array.Empty<string>(), new int[] { 1, 2, 3, 4, 5 }, new double[] { 5, 4, 5, 4, 1 }, 0.25, 0.3, 0.0, 0.08, null, true)
            .Register("Somali");

        public static readonly LanguageGen HindiRomanized = new LanguageGen(
                new string[]{
                ...
                Array.Empty<string>(), new int[] { 1, 2, 3, 4, 5 }, new double[] { 1, 2, 3, 3, 1 }, 0.15, 0.75, 0.0, 0.12, null, true)
            .Register("Hindi Romanized");

        public static readonly LanguageGen ArabicRomanized = new LanguageGen(
                ...
                Array.Empty<string>(), new int[] { 1, 2, 3, 4 }, new double[] { 6, 5, 5, 1 }, 0.55, 0.65, 0.0, 0.15, ArabicSanityChecks, true)
            .Register("Arabic Romanized");
        ...
        public static readonly LanguageGen Inuktitut = new LanguageGen(
                ...
                Array.Empty<string>(), new int[] { 1, 2, 3, 4, 5 }, new double[] { 3, 4, 6, 5, 4 }, 0.45, 0.0, 0.0, 0.25, null, true)
            .Register("Inuktitut");
        ...
        public static readonly LanguageGen Norse = new LanguageGen(
                ...
                Array.Empty<string>(), new int[] { 1, 2, 3, 4, 5 }, new double[] { 5, 5, 4, 3, 1 }, 0.25, 0.5, 0.0, 0.08, GenericSanityChecks, true)
            .Register("Norse");
        ...
        public static readonly LanguageGen Nahuatl = new LanguageGen(
                ...
                .AddModifiers(new Modifier("c([ie])", "qu$1"), new Modifier("z([ie])", "c$1"))
            .Register("Nahuatl");
        ...
        public static readonly LanguageGen Mongolian = new LanguageGen(
                ...
                Array.Empty<string>(), new int[] { 1, 2, 3, 4 }, new double[] { 5, 9, 3, 1 }, 0.3, 0.2, 0.0, 0.07, null, true)
            .Register("Mongolian");
        ...
        public static readonly LanguageGen Goblin = new LanguageGen(
                ...
                Array.Empty<string>(),
                Array.Empty<string>(), new int[] { 1, 2, 3, 4 }, new double[] { 3, 7, 5, 1 }, 0.1, 0.15, 0.0, 0.0, GenericSanityChecks, true)
            .Register("Goblin");
        ...
        public static readonly LanguageGen Elf = new LanguageGen(
                ...
                Array.Empty<string>(),
                Array.Empty<string>(), new int[] { 1, 2, 3, 4, 5 }, new double[] { 3, 6, 6, 3, 1 }, 0.4, 0.3, 0.0, 0.0, GenericSanityChecks, true)
            .Register("Elf");
        ...
        public static readonly LanguageGen AlienA = new LanguageGen(
                ...
                Array.Empty<string>(),
                Array.Empty<string>(), new int[] { 1, 2, 3 }, new double[] { 1, 1, 1 }, 0.65, 0.6, 0.0, 0.0, null, true)
            .Register("Alien A");

        public static readonly LanguageGen KoreanRomanized = new LanguageGen(
                new string[]{
                ...
                null, true)
            .Register("Korean Romanized");

        public static readonly LanguageGen AlienE = new LanguageGen(
                ...
                Array.Empty<string>(),
                Array.Empty<string>(), new int[] { 1, 2, 3 }, new double[] { 5, 4, 2 }, 0.45, 0.0, 0.0, 0.0, null, true)
            .Register("Alien E");

        public static readonly LanguageGen AlienI = new LanguageGen(
                new string[]{
        ...
        public static readonly LanguageGen AlienO = new LanguageGen(
                new string[]{
                    "a", "e", "i", "o", "o", "o", "o", "u",
                ...
                Array.Empty<string>(),
                Array.Empty<string>(), new int[] { 1, 2, 3 }, new double[] { 3, 6, 4 }, 0.0, 0.55, 0.0, 0.0, null, true)
            .Register("Alien O");

        // àáâãäåæāăąǻǽaèéêëēĕėęěeìíîïĩīĭįıiòóôõöøōŏőœǿoùúûüũūŭůűųuýÿŷỳ
        // çðþñýćĉċčďđĝğġģĥħĵķĺļľŀłńņňŋŕŗřśŝşšţťŵŷÿźżžșțẁẃẅ
        public static readonly LanguageGen AlienU = new LanguageGen(
                new string[]{
        ...
        public static readonly LanguageGen Dragon = new LanguageGen(
                ...
                Array.Empty<string>(), new int[] { 2, 3, 4, 5 }, new double[] { 2, 7, 10, 3 }, 0.14, 0.04, 0.0, 0.11, GenericSanityChecks, true)
            .Register("Dragon");

        public static readonly LanguageGen Kobold = new LanguageGen(
                Dragon.GetOpeningVowels(), Dragon.GetMidVowels(), Dragon.GetOpeningConsonants(), Dragon.GetMidConsonants(), Dragon.GetClosingConsonants(),
                new string[] { "ik", "ak", "ek", "at", "it", "ik", "ak", "ek", "at", "it", "ik",
                    "ak", "ek", "at", "it", "et", "ut", "ark", "irk", "erk" },
                Array.Empty<string>(), new int[] { 1, 2, 3 }, new double[] { 5, 11, 1 }, 0.1, 0.0, 0.0, 0.22, GenericSanityChecks, true)
            .Register("Kobold");
        ...
        public static readonly LanguageGen Insect = new LanguageGen(
                ...
                Array.Empty<string>(),
                Array.Empty<string>(), new int[] { 1, 2, 3, 4 }, new double[] { 6, 4, 2, 1 }, 0.3, 0.1, 0.0, 0.0, null, true)
            .Register("Insect");
        ...
        public static readonly LanguageGen Maori = new LanguageGen(
                ...
                new string[] { "" },
                Array.Empty<string>(),
                Array.Empty<string>(), new int[] { 1, 2, 3, 4 }, new double[] { 5, 5, 4, 2 }, 0.2, 1.0, 0.0, 0.0, GenericSanityChecks, true)
            .Register("Maori");
        ...
        public static readonly LanguageGen DeepSpeech = new LanguageGen(
                new string[]{
                    "a", "a", "o", "o", "o", "o", "u", "u", "u", "u",
                    "a", "a", "o", "o", "o", "o", "u", "u", "u", "u",
                    "a", "a", "o", "o", "o", "o", "u", "u", "u", "u",
            ...
            .Register("Deep Speech");

        public static readonly LanguageGen NorseSimplified = Norse
            .AddModifiers(Modifier.ReplacementTable(
                "á", "a",
                "é", "e",
                "í", "i",
                "ý", "y",
        ...
        public static readonly LanguageGen Hletkip = new LanguageGen(
                ...
                Array.Empty<string>(), new int[] { 1, 2, 3 }, new double[] { 1, 1, 1 }, 0.0, 0.4, 0.0, 1.0, null, true)
            .Register("Hletkip");

        public static readonly LanguageGen AncientEgyptian = new LanguageGen(
        ...
        public static readonly LanguageGen Malay = new LanguageGen(
                ...
                Array.Empty<string>(), new int[] { 1, 2, 3 }, new double[] { 5, 3, 2 }, 0.2, 0.25, 0.0, 0.2, GenericSanityChecks, true)
            .Register("Malay");
        ...
        public static readonly LanguageGen ChineseRomanized = new LanguageGen(
                new string[]{
                ...
                Array.Empty<string>(),
                Array.Empty<string>(), new int[] { 1, 2, 3 }, new double[] { 14, 3, 1 }, 0.175, 0.55, 0.0, 0.0, GenericSanityChecks, true)
            .Register("Chinese Romanized");

        public static readonly LanguageGen CherokeeRomanized = new LanguageGen(
                new string[]{
                ...
                Array.Empty<string>(),
                Array.Empty<string>(), new int[] { 1, 2, 3, 4 }, new double[] { 4, 7, 6, 2 }, 0.3, 0.96, 0.0, 0.0, null, true)
            .Register("Cherokee Romanized");
        ...
        public static readonly LanguageGen Fantasy = Malay
            .Mix(2, (French, 4), (Simplish, 1), (JapaneseRomanized, 2), (Elf, 2), (GreekRomanized, 1), (Celestial, 1))
            .RemoveAccents()
            .Register("Fantasy");

        public static readonly LanguageGen FancyFantasy = Fantasy
            .AddAccents(0.47, 0.07)
            .Register("Fancy Fantasy");
